<template>
    <v-sheet
        class="block-base"
        :class="{'click': isModifiable, 'noclick': !isModifiable, 'active': isActiveBlock, 'frozen': isFrozenGame}"
        rounded
    ...
    >
        <div
            v-if="properties"
            :style="baseImage + styleString"
            class="block-image"
            ><div class="aspect-ratio"></div></div>
        <div v-if="properties && overlayIcon"
            class="overlay">
            <v-btn
              class="extra-static"
              elevation="2"
...
export default {
    ...
    computed: {
        isCurved () {
            ...
            const diff = Math.abs(this.properties.Connections[0] - this.properties.Connections[1]);
...
div.block-base.active, div.block-base.active:hover{
    cursor: default;
    background-color: #00695C !important; /* teal darken-3 */
}

div.block-base.active.noclick, div.block-base.active.noclick:hover{
    cursor: not-allowed;
    background-color: #c62828bb !important; /* red lighten-1 */
}

div.block-base.frozen:not(.active) { /* Override everything */
